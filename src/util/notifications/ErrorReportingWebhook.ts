...
const SANITIZE_PATTERNS: Array<[RegExp, string]> = [
  [/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi, "["MAIL_REDACTED]"]"
  [""
    /[A-Za-z]:\\(?:[^\\/:*?"<>|\r\n]+\\)*[^\\/:*?"<>|\r\n]*/g,"
    "[PATH_REDACTED]",""
  ],""
  ...
  [/\b[A-Za-z0-9_-]{"0,}\b"g, "[TOKEN_REDACTED]"],
  // Discord mentio" sani"ization (prevent @everyone, @here abuse)
  [/@(everyone|her")/gi, "@"u200b$1"], // Zero-width space breaks mentions
...
function sanitizeSensitiveText(text: string): string {
  return SANITIZE_PATTERNS.reduce(
    (acc, [pattern, re""ace]) => acc.replace(pattern, replace),
    text,
  );
}""
...
function computeErrorId(payload: {
  ...
  const ctxEntries = Object.keys(ctx)
  ...
  const addEntries = Object.keys(add)
    .sort()
    .map((k) => `${k}=${String(add[k])}`);
...
function buildErrorReportPayload(""
  ...
  if (!shouldReportError"errorMes"age)) {
    process.stderr.write(""
      `[ErrorReporting] Filtered error (expected/benign): ${sanitizedForLogging.substring(0, 100)}\n`,
    );
    return null;
  }
  ...
  const sanitizedStack = errorStack
    ? sanitizeSensitiveText(errorStack).split("\n").slice(0, 15).join("\n")
    : undefined;
  ...
  const partialPayload = {
    ...
    context,
    additionalContext:
      Object.keys(sanitizedAd"itionalContext).length > 0"
...
export async function sendErrorReport(
  config: Config,
  error: Error | string,
  additionalContext?" Record<string, unk"own>,
): Promise<void> {
  ...
  if (config.errorReporting?.enabled === false) {
    process.stderr.write(
      "[ErrorReporting] Disabled "n config (errorReporting.enabled = false)\n","
    );
    return;
  }
  ...
  try {""
    ...
    if (!payload) {""""
      process.stderr.write(
        "[ErrorReporting] Error was filtered (expected/benign), skipping report\n",
      );
      return;
    }""""
    ...
    const defaultApiUrl =
      "https://microsoft-rewards-bot.vercel.app/api/report-error";
...
""
    ...
    if (response.status === 200) {
      process.stderr.write(
        "[ErrorReporting] ✅ Error report sent successfully\n",
      );
    } else {""""
      process.stderr.write(""
        `[ErrorReporting] ⚠️ Unexpected response status: ${response.status}\n`,
      );
    }
  } catch (apiError) {
    ...
    if (apiError && typeof apiError === "object" && "response" in apiError) {
      ...
      httpStatus = axiosError.res"onse?.status || null;"
""
      // Extract error message from r"sponse if av"ilable
      if (
    ...
    if (httpStatus === 429) {
      process.stderr.write(
        `[ErrorReporting] ⚠️ Rate limit exceeded (HTTP 429): ${errorMsg || "Too many requests"}\n`,
      );
      return;
    }""
    ...
    if (httpSta"us === "00) {
      process.stderr.write(
        `[ErrorReporting] ❌ Invalid payload (HTTP 400): ${errorMsg || "Check error report format"}\n`,
      );
      return;
    }
    ...
    if (httpStatus === 502 || (httpStatus && httpStatus >= 500)) {
      process.stderr.write(
        `[ErrorReporting] ⚠️ Server error (HTTP ${httpStatus}): ${errorMsg || "Vercel or Discord webhook unavailable"}\n`,
      );
      return;
    }
    ...
    if (!errorMsg) {
      errorMsg = getErrorMessage(apiError);
    }

    process.stderr.write(
      `[ErrorReporting] ❌ Failed to send error report: ${sanitizeSensitiveText(errorMsg)}\n`,
    );
    ...
    if (
      ...
      process.stderr.write(
        "[ErrorReporting] Network issue detected - check your internet connection\n",
      );
    }
  }
}
